...
while not rospy.is_shutdown():
   ...
   # elif state == 1:
    ...
    if state == 0:
        ...
        print "left"
        if rospy.Time.now()-time_prev > rospy.Duration(4.18):
            state = 3
            time_prev = rospy.Time.now()
        ...
        command.angular.z = 0.0
        print "foward"
        if rospy.Time.now()-time_prev > rospy.Duration(2.05):
            state = 4
            time_prev = rospy.Time.now()
        ...
        command.angular.z = 0.5
        print "left"
        if rospy.Time.now()-time_prev > rospy.Duration(3.17):
            state = 9
            time_prev = rospy.Time.now()
        ...
        command.angular.z = 0.0
        print "foward"
        if rospy.Time.now()-time_prev > rospy.Duration(4.5):
            state = 10
            time_prev = rospy.Time.now()
        ...
        command.angular.z = 0.0
        print "foward"
        if rospy.Time.now()-time_prev > rospy.Duration(6.1):
            state = 12
            time_prev = rospy.Time.now()
            
    elif state ==12:                                                  #left
        command.linear.x = 0.12
        command.angular.z = 0.37
        print "left"
        if rospy.Time.now()-time_prev > rospy.Duration(4.25):
            state = 13
            time_prev = rospy.Time.now()
        ...
        command.angular.z = 0.0
        print "foward"
        if rospy.Time.now()-time_prev > rospy.Duration(3.05):
            state = 20
            time_prev = rospy.Time.now()
        ...
        command.angular.z = 0.39
        print "left turn"
        if rospy.Time.now()-time_prev > rospy.Duration(4.16):
            state = 21
            time_prev = rospy.Time.now()
        ...
        command.angular.z = 0.0
        print "foward"
        if rospy.Time.now()-time_prev > rospy.Duration(7.05):
            state = 22
            time_prev = rospy.Time.now()
